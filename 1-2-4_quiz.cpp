...
int main(int argc, char** argv) {
	...
	//Create the window
	glutCreateWindow("Muhammad Ilham Surya 181511025");
	initRendering();
...
void update(int value){
    ...
    for(int i = 0; i < numTanks; i++){
		...
		int z = 0;
		while (distanceBetween(0, 0, x, z) < 20) {
			x = rand() % (1*mapSize) - mapSize;
			z = rand() % (1*mapSize) - mapSize;
		}
		createTank(float(x), float(z));
		maxtank++;
		if (! tanks[i]->canMoveTo(tanks[i]->givePosX(), tanks[i]->givePosZ())){
...
//	_textureBricks = loadTexture("bricks.bmp");
//	_textureRoof = loadTexture("roof.bmp");
//	_textureDoor = loadTexture("door.bmp");
//	_textureWindow = loadTexture("jendela2.bmp");
//	_textureWallUp = loadTexture("tembokatas.bmp");
//	_textureWallDown = loadTexture("tembokputih.bmp");
//	_textureJendela = loadTexture("window.bmp");
//	_textureTembokAlas = loadTexture("_bagianbawah.bmp");	
//	_textureAtas = loadTexture("atas.bmp");
//	_textureTiang = loadTexture("batubeton.bmp");
}
